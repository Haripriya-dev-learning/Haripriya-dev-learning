...
ResultSet rs = st.executeQuery("select * from keyrequests where PANID='"+p+"' and
DESCR='"+description+"'");
if(rs.next())
{
out.println("<html><body><script>alert('Key Already
Requested');</script></body></html>");
...
keyrequests(NAME,EMAIL,DESCR,PANID,DATE) values(?,?,?,?,?)");
ps.setString(1,n);
ps.setString(2,e);
ps.setString(3,d);
ps.setString(4,p);
...
request.getRequestDispatcher("Emphome.jsp");
